...
class ModeSelectionEnv(gym.Env):
    metadata = {
        "render_modes": [],
        "render_fps": 60,
    }
    ...
    def __init__(self,
                 loco_model: PPO = None,
                 loco_env: gym.Env = None,
                 **kwargs):
        self.loco_model = loco_model
        self.loco_env = loco_env
        self.init_mode = np.eye(len(mode))[0]
        self._init_space()
    ...
    def step(self, action):
        ...
        self.loco_obs["mode_vector"] = action
        loco_obs = np.concatenate([value for value in self.loco_obs.values()])
        loco_action, _ = self.loco_model.predict(loco_obs, deterministic=True)
