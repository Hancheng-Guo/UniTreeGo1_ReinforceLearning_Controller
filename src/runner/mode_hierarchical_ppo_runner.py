...
from src.callback.base import TrainProgressCallback, TestProgressCallback, IterProgressCallback
...
class ModeHierarchicalPPORunner(PPOTrainer, PPOTester):
    ...
    def train(self,
        ...
        self.learn(loco_callback=[IterProgressCallback(**self.loco_callback_kwargs),
                                  CustomTensorboardCallback(**self.loco_callback_kwargs),
                                  AdaptiveLRCallback(**self.loco_callback_kwargs)],
                   mode_callback=[IterProgressCallback(**self.mode_callback_kwargs),
                                  CustomTensorboardCallback(**self.mode_callback_kwargs),
    ...
    def learn(self,
        ...
        self.loco_callback = IterCallBackList([*com_callback,*loco_callback])
        self.loco_callback.init_callback(self.loco_model)
        self.mode_callback = IterCallBackList([*com_callback,*mode_callback])
        ...
        while iteration < self.total_iterations:
            self.loco_callback.on_iteration_start()
            self.mode_callback.on_iteration_start()

            tmp = None
            loco_collect_time = 0
            loco_total_time = None
            mode_collect_time = 0
            mode_total_time = None

            loco_total_time = time.perf_counter()
            while True:
                ...
                loco_collect_time += time.perf_counter() - tmp
                ...
                if self.loco_model.num_timesteps % self.loco_iteration_steps == 0:
                    break
            loco_total_time = time.perf_counter() - loco_total_time

            mode_total_time = time.perf_counter()
            while True:
                tmp = time.perf_counter()
                ...
                mode_collect_time += time.perf_counter() - tmp
                ...
                if self.mode_model.num_timesteps % self.mode_iteration_steps == 0:
                    break
            mode_total_time = time.perf_counter() - mode_total_time


            print(f"loco: {(loco_collect_time / loco_total_time * 100):.2f}%")
            print(f"mode: {(mode_collect_time / mode_total_time * 100):.2f}%")
        ...
        self.mode_callback.on_training_end(locals(), globals())
    ...
    def get_callback_kwargs(self):
        ...
        self.loco_callback_kwargs = {
            # for ProgressBar
            "rollout_steps": loco_rollout_steps,
            "custom_str": "Loco-Rollout",
            "n_rollouts": self.config["train"]["total_iterations"] * loco_iteration_rollouts,
        ...
        self.mode_callback_kwargs = {
            # for ProgressBar
            "rollout_steps": mode_rollout_steps,
            "custom_str": "Mode-Rollout",
            "n_rollouts": self.config["train"]["total_iterations"] * mode_iteration_rollouts,
            # for CustomTensorboardCallback
